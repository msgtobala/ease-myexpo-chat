...
interface Post {
  ...
  postComments: Comment[]; // Array of comment objects
  postCommentsCount: number; // Total number of comments
}
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          postsData.push({
            ...data,
            likedBy: data.likedBy || [], // Ensure likedBy array exists
            postComments: normalizedComments, // Ensure comments array exists
            postCommentsCount: data.postCommentsCount || normalizedComments.length // Ensure comments count exists
          } as Post);
        });
        setPosts(postsData);
        setLoading(false);
      ...
      (error) => {
        ...
        if (error.message?.includes("Failed to fetch")) {
          console.warn("Network connection issue detected. Posts may not be up to date.");
        }
      },
    );
    ...
    try {
      ...
      if (isCurrentlyLiked) {
        ...
        await updateDoc(postRef, {
          ...
          postLikes: Math.max(0, currentLikedBy.length - 1), // Ensure non-negative
        });
      } else {
        // Like the post
        await updateDoc(postRef, {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div
                    ...
                    onClick={() =>
                      handleProfileClick(post.postedBy, post.postedByType)
                    }
                  >
                    {post.postedBy}
                  </div>
