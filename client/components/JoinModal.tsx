...
export default function JoinModal({ isOpen, onClose, userProfileType, exhibitionData }: JoinModalProps) {
  ...
  useEffect(() => {
    if (isOpen && (userProfileType === 'visitor' || userProfileType === 'exhibitor')) {
      fetchIndustries();
    }
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      const userCompanyLogoUrl = userData.companyLogoUrl || userData.profilePictureUrl || '';
      const userBrochureUrl = userData.brochureUrl || '';

      // 1. Update exhibition's joined_profile field
      const exhibitionDocRef = doc(db, 'exhibitions', exhibitionData.exhibition_id);
        ...
        <div className="flex gap-3 p-6 border-t border-gray-100">
          ...
          <button
            ...
            disabled={
              ...
              (userProfileType === 'exhibitor' && (!selectedIndustry || !hallNumber)) ||
              joining
            }
